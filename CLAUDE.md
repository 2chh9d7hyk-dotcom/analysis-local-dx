...
- **フェーズ2（次）**: パブリックWebアプリ化（Streamlit Cloud）← **ここが急務**
- **フェーズ3（実装済み）**: e-Stat / RESAS APIとの自動連携 ← セッション6で先行実装
- **フェーズ4**: 認証・アクセス制御（一般公開 vs 庁内限定データ分離）
...
## 2. ディレクトリ構成（2026-06-27 セッション6 完了時点）

```
c:\analysis_local_dx\
├── CLAUDE.md                              ← このファイル
├── requirements.txt                       ← streamlit/pandas/plotly/scikit-learn/mlxtend/requests/python-dotenv/pillow
├── .env.example                           ← APIキーテンプレート ★セッション6追加
...
│   └── data_management.py                ← データ管理（5タブ：CSV/確認/SQLite/API/スキーマ）
├── src/
│   ├── __init__.py
...
│   │   ├── db.py                          ← SQLite永続化レイヤー
│   │   └── api_client.py                  ← e-Stat / RESAS APIクライアント ★セッション6追加
│   └── analytics/
│       ├── __init__.py
...
└── data/
    ├── local.db                           ← SQLite永続DB（自動生成）
    ├── reference/
    │   └── national_avg.csv              ← 全国・県平均参照データ（2021〜2024年）★セッション6追加
    └── sample/
...
| data/local.db の gitignore | ⚠️ 未対応 | Streamlit Cloud デプロイ前に必ず `.gitignore` に追加すること |
...
**足りないのは、機能ではなく「体験できる場所」だ。**

NES面接で最も効果的なのは「URLをお渡しします。触ってみてください」の一言。
ローカルで動くシステムをスクリーンショットで説明するより、生きたURLは100倍の説得力を持つ。

**Streamlit Cloud へのデプロイ（6-11）が次の最優先タスク。**
...
#### **6-11. Streamlit Cloud デプロイ**

**なぜ最優先か**: ポートフォリオとして「生きたURL」は他のどの機能追加より価値が高い。

実施内容:
1. `.gitignore` の作成（`data/local.db`, `.env` を除外）
2. `data/sample/` のサンプルデータが git に含まれているか確認
3. Streamlit Cloud (share.streamlit.io) に GitHub連携でデプロイ
4. Secrets に `RESAS_API_KEY`, `ESTAT_API_KEY` を設定（オプション）
5. デプロイ後の動作確認（サンプルデータで全ページが動くこと）

注意点:
- `data/local.db` は永続化できないのでリセットされる（サンプルデータへのフォールバックで対応済み）
- `mlxtend` が Streamlit Cloud で動くか確認が必要（requirements.txtに記載済みなので問題ないはず）
...
#### **6-9. 認証・公開制御**
デプロイ後、庁内限定データ（activity_logs等）を保護するために必要。
- `streamlit-authenticator` でログイン機能
- 一般公開データ（人口・NES指標）と庁内限定データ（アクションログ）のルーティング分離

#### **6-10. テスト追加**
NES面接で「エンジニアリングの信頼性」を示すために有効。
- `tests/test_rfm.py`, `tests/test_basket.py`, `tests/test_churn.py`, `tests/test_cascade.py`
- GitHub Actions での CI パイプライン（`pytest` + `python -m ast` 構文チェック）

---

### 🟢 低優先（フェーズ3以降）

- **RESAS cityCode 変換の精度向上**: `split_code()` の変換ロジックを全自治体で検証・修正
- **e-Stat 国勢調査の深掘り**: `fetch_population_by_municipality()` で年齢別人口から高齢化率を自動計算
- **NES指標のプロキシ推定**: e-Stat のデジタル関連指標からオンライン化スコアを自動算出
- **activity_logs の実データフォーマット対応**: 各自治体が持つシステムのCSV形式に対応するコンバーター
...
- **次（S7）**: Streamlit Cloud デプロイ（6-11）から着手
